...
- **Chat Modes**: Standard (general chat), Deep Research (auto web search + multi-query synthesis), Coding (code-focused responses with best practices).
- **File Uploads**: Upload CSV, JSON, TXT, PDF, code files, or images. Omega reads and analyzes them — parses tables, extracts insights, summarizes documents.
...
- **Google Drive Sync**: Chat sessions and memories are stored in your personal Google Drive in a folder unique to YOUR email address (OmegaCloudData_your_email). This folder is STRICTLY per-user — each Google account gets its own isolated folder. No one else — not other testers, not other group members, not other app users — can access your data. Only you, signed into your specific Google account, can see or modify your sessions and memories. Data syncs across devices when you're signed into the same account. Omega can reference Drive storage for persistent memory and chat history, and can suggest the user save important information as memories for future sessions.
