...
// ── Terminal component inside the panel ────────────────────────────────
...
export function PCRemotePanel() {
  ...
  const [lines, setLines] = React.useState<TerminalLine[]>([
    {
      id: ++lineId,
      text: `Ω PC Remote Agent — ${status === "online" ? "Connected" : "Disconnected"}`,
      type: "system",
    },
  ]);
  ...
  // Add system line when status changes
  React.useEffect(() => {
    setLines((prev) => [
      ...prev,
      {
        id: ++lineId,
        text: `Status: ${status}${hostname ? ` — ${hostname}` : ""}`,
        type: "system",
      },
    ]);
  }, [status, hostname]);
      ...
      >
        <span
          className={cn(
            ...
            status === "online" && "text-[var(--omega-emerald)]",
            status === "offline" && "text-[var(--omega-rose)]",
            status === "connecting" && "text-[var(--omega-amber)]"
          )}
        >
          <span
            className={cn(
              "absolute inline-flex h-full w-full animate-ping rounded-full opacity-70",
              status === "online" && "bg-[var(--omega-emerald)]",
              status === "offline" && "bg-[var(--omega-rose)]",
              status === "connecting" && "bg-[var(--omega-amber)]"
            )}
          />
          <span
            className={cn(
        ...
        {status === "offline" && (
          <button
            type="button"
            onClick={reconnect}
            className="inline-flex items-center gap-1 rounded-md px-2 py-1 text-[10px] text-[var(--omega-fg-dim)] hover:text-[var(--omega-emerald)]"
          >
      ...
      {status === "online" && (
        <div className="flex items-center gap-1 border-b border-[var(--omega-glass-border)] px-2 py-1.5">
          <button
            ...
            className="inline-flex items-center gap-1 rounded-md px-2 py-1 text-[10px] text-[var(--omega-fg-dim)] transition-colors hover:bg-[oklch(0.82_0.17_162_/_0.1)] hover:text-[var(--omega-emerald)] disabled:opacity-40"
          >
            {explorerBusy ? (
          ...
          <button
            type="button"
            onClick={() => {
              addLine("System info:", "system");
              exec("systeminfo | findstr /B /C:\"OS Name\" /C:\"OS Version\" /C:\"System Type\"")
                .then((r) => {
          ...
          >
            <div className="px-3 py-2">
              <div className="mb-1 flex items-center gap-1">
                <button
                  type="button"
                  onClick={explorerGoUp}
                  className="rounded px-1.5 py-0.5 text-[10px] text-[var(--omega-fg-dim)] hover:text-[var(--omega-emerald)]"
                >
                  ↑ ..
                </button>
                <span className="flex-1 truncate font-mono text-[9px] text-[var(--omega-muted)]">
                  {explorerPath}
                </span>
                <button
                  type="button"
                  onClick={() => setShowExplorer(false)}
                  className="rounded px-1 py-0.5 text-[var(--omega-fg-dim)] hover:text-[var(--omega-rose)]"
                >
              ...
              <div className="max-h-32 overflow-y-auto rounded-lg bg-[var(--omega-bg-2)] p-1">
                {explorerBusy ? (
                  <div className="flex items-center justify-center py-4">
                    <Loader2 className="size-4 animate-spin text-[var(--omega-muted)]" />
                  </div>
                ) : explorerItems.length === 0 ? (
                  <div className="py-2 text-center text-[10px] text-[var(--omega-muted)]">
                    Empty directory
                  </div>
                ) : (
                  explorerItems.map((item) => (
                    <button
                      key={item}
                      type="button"
                      onClick={() => explorerSelect(item)}
                      className="flex w-full items-center gap-1.5 rounded-md px-2 py-1 text-left text-[10px] text-[var(--omega-fg-dim)] transition-colors hover:bg-[oklch(0.82_0.17_162_/_0.1)] hover:text-[var(--omega-fg)]"
                    >
        ...
        <div className="omega-glass-thin flex items-center gap-2 rounded-lg px-2.5 py-1.5">
          ...
          <input
            ...
            placeholder={
              status === "online"
                ? "Enter command..."
                : "PC offline — waiting for connection..."
            }
